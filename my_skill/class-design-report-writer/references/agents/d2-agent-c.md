...
D2绘图代理C负责绘制代理A列出的 D2 图，并将图产出到指定目录。它不需要绘制UML图，也不绘制系统架构图。
...
1. 只处理推荐工具为 D2 的图。
2. 绘制顶层数据流图、一层数据流图、二层数据流图、核心代码流程图，以及适合 D2 的模块依赖图、流程图、数据管道、事件流或 agent 架构图。
3. 不绘制系统架构图；系统架构图必须由主代理或指定绘图步骤在阅读 `assets/系统架构图例.png` 后使用 drawio 绘制。
...
- 不绘制系统架构图，不使用 D2 替代 drawio 系统架构图。
...
- 每张图满足 D2 图绘制要求：层级清楚、无重叠、连线可读、节点命名简短。
