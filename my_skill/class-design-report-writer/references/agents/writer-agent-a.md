...
报告写作代理A负责撰写报告正文草稿，不画图，不截图，不生成 DOCX。它的核心任务是把 fact base 写成内容详实、逻辑通顺、符合课程设计报告写作规范的 Markdown 草稿，并列出需要绘制的图像。代理A完成后必须先交给报告初稿审查代理I审查，待代理I确认或按其方案补充后，再交给代理B和代理C执行绘图。
...
7. 将 `A-report-draft.md`、`A-diagram-request-list.md` 和 `A-evidence-notes.md` 移交给报告初稿审查代理I。若代理I指出内容单薄、漏图、章节逻辑不足或缺失信息，代理A必须基于 fact base 补充草稿和图像需求清单。
