...
4. 分发给子代理完成 A-F 角色任务，并在 prompt 中明确输入、输出目录、可读 references、禁止越界事项和停止标准。
5. 收回子代理产物后进行冲突处理，确保报告正文、图像、截图和证据图谱一致。
6. 在最终交付前运行 skill 验收清单，确认 Markdown、DOCX、图片、测试证据和 evidence map 均存在。
...
- 若当前环境不能启动子代理，主代理必须按 A-F 角色顺序模拟执行，并在 evidence map 中记录“环境未提供子代理工具，已按相同角色边界顺序执行”。
...
- A-F 子代理产物齐全，且每项产物能映射到 fact base。
